...
The GitHub Pages demo does not run dbt or publish `profiles.yml`. It builds static review docs from the generated MDX, design, mapping, and validation files:

```bash
cd southern_cross_bank_demo_pipeline
python scripts/build_static_docs.py . site
```
...
- The GitHub Pages workflow performs profile-free static documentation publishing.
