...
SKIP_DIRS = {".git", "node_modules", "target", "dbt_packages", "build", ".docusaurus"}
...
def check_mdx(root: Path, report: Report) -> None:
    docs_dir = root / "docs"
    mdx_files = sorted(docs_dir.rglob("*.mdx")) if docs_dir.is_dir() else []
    if not mdx_files:
        report.error("no Docusaurus MDX file found under docs/")
        return
    ...
    for path in mdx_files:
        ...
        if mermaid_blocks == 0:
            report.error(f"{relative}: missing Mermaid diagram fence")
        elif mermaid_blocks < 2:
            report.warn(f"{relative}: expected at least two Mermaid diagrams")
...
def check_github_actions(root: Path, report: Report) -> None:
    ...
    for path in workflows:
        ...
        if "actions/configure-pages" not in text:
            report.error(f"{relative}: missing configure-pages action")
        ...
        if "npm run build" not in text and "yarn build" not in text and "pnpm" not in text and "bun" not in text:
        ...
        for current, replacement in outdated_actions.items():
            if current in text:
                report.warn(f"{relative}: update {current} to {replacement}")
