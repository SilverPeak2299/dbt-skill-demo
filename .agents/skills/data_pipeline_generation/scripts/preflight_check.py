...
def preflight(root: Path, args: argparse.Namespace) -> tuple[list[Finding], dict[str, Any]]:
    ...
    if "@docusaurus/core" in deps:
        ...
        findings.append(
            Finding(
                ...
                "Required for building the generated MDX page in a Docusaurus site.",
            )
        )
    ...
    if build_script:
        ...
        findings.append(
            Finding("warn", "docs build command", "package.json has no build script", "Add a Docusaurus build script.")
        )
    ...
    mermaid_supported = "@docusaurus/theme-mermaid" in deps or "mermaid: true" in config_text
    facts["mermaid"] = mermaid_supported
    if mermaid_supported:
        findings.append(Finding("pass", "Mermaid", "Mermaid support detected"))
    else:
        findings.append(
            Finding(
                "warn",
                "Mermaid",
                "Mermaid support not detected",
                "Add @docusaurus/theme-mermaid and markdown.mermaid: true, or confirm Mermaid rendering is configured elsewhere.",
            )
        )
    ...
    if pages_workflows:
        ...
        findings.append(
            Finding(
                ...
                "Generate one from assets/github-actions/publish-docusaurus.yml if docs publishing is required.",
            )
        )
